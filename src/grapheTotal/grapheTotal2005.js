...
var width2005 = 400, height2005 = 350;
d3.select("#grapheTotal2005")
  .append("svg")
  .attr("id","svgComparatifPaysCause2005")
  .attr("width", "100%")
  .attr("height", height2005)
  //.attr("viewBox", [0,0,800,700])
  .attr("viewBox", [300,-20,200,500])
  .attr("font-size","0.9rem")

var svg2005 = d3.select("#svgComparatifPaysCause2005"),
// width2005 = +svg2005.attr("width2005"),
// height2005 = +svg2005.attr("height2005"),
innerRadius2005 = 100,
outerRadius2005 = Math.min(width2005, height2005),
g2005 = svg2005.append("g").attr("transform", "translate(" + width2005 + "," + height2005 + ")");

var x2005 = d3.scaleBand()
.range([0, 2 * Math.PI]);

var y2005 = d3.scaleRadial()
.range([innerRadius2005, outerRadius2005]);

var z2005 = d3.scaleOrdinal()
...
x2005.domain(parsedData.map(function(d) { return d.pays; }));
y2005.domain([0, d3.max(parsedData, function(d) { return d.total; })]);
z2005.domain(parsedData.columns.slice(1));

var tooltip2005 = d3.select("#grapheTotal2005")
        .append("div")
        .attr("class", "tooltip")
...
.enter().append("g")
.attr("fill", function(d) { return z2005(d.key); })
.selectAll("path")
.data(function(d) { return d; })
.enter().append("path")
.attr("d", d3.arc()
    .innerRadius(function(d) { return y2005(d[0]); })
    .outerRadius(function(d) { return y2005(d[1]); })
    .startAngle(function(d) { return x2005(d.data.pays); })
    .endAngle(function(d) { return x2005(d.data.pays) + x2005.bandwidth(); })
    .padAngle(0.01)
    .padRadius(innerRadius2005))
    .on('mouseover', function(event,d) {
      //console.log(event.selection)
       tooltip2005.style("visibility", "visible")
     })
     ...
     .on('mouseout', function() {
       tooltip2005.style("visibility", "hidden")
     });

/*var label2005 = g2005.append("g")
.selectAll("g")
.data(parsedData)
.enter().append("g")
.attr("text-anchor", "middle")
.attr("transform", function(d) { return "rotate(" + ((x2005(d.pays) + x2005.bandwidth() / 2) * 180 / Math.PI - 90) + ")translate(" + innerRadius2005 + ",0)"; });

label2005.append("line")
.attr("x2", -5)
.attr("stroke", "#000");

label2005.append("text")
.attr("transform", function(d) { return (x2005(d.pays) + x2005.bandwidth() / 2 + Math.PI / 2) % (2 * Math.PI) < Math.PI ? "rotate(90)translate(0,16)" : "rotate(-90)translate(0,-9)"; })
.text(function(d) { return d.pays; });*/

var yAxis2005 = g2005.append("g")
.attr("text-anchor", "end");

var yTick2005 = yAxis2005
.selectAll("g")
.data(y2005.ticks(7).slice(1))
.enter().append("g");

yTick2005.append("circle")
.attr("fill", "none")
.attr("stroke", "#000")
.attr("stroke-opacity", 0.5)
.attr("r", y2005);

yTick2005.append("text")
.attr("x", -6)
.attr("y", function(d) { return -y2005(d); })
.attr("dy", "0.35em")
.attr("fill", "none")
.attr("stroke", "#fff")
.attr("stroke-linejoin", "round")
.attr("stroke-width2005", 3)
.text(y2005.tickFormat(10, "s"));
...
yAxis2005.append("text")
.attr("x", -6)
.attr("y", function(d) { return -y2005(y2005.ticks(10).pop()); })
.attr("dy", "-1em")
