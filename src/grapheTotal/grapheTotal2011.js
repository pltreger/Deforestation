...
var width2011 = 400, height2011 = 350;
d3.select("#grapheTotal2011")
  .append("svg")
  .attr("id","svgComparatifPaysCause2011")
  .attr("width", "100%")
  .attr("height", height2011)
  //.attr("viewBox", [0,0,800,750])
  .attr("viewBox", [300,-30,200,500])
  .attr("font-size","0.9rem")

var svg2011 = d3.select("#svgComparatifPaysCause2011"),
// width2011 = +svg2011.attr("width2011"),
// height2011 = +svg2011.attr("height2011"),
innerRadius2011 = 100,
outerRadius2011 = Math.min(width2011, height2011),
g2011 = svg2011.append("g").attr("transform", "translate(" + width2011 + "," + height2011 + ")");

var x2011 = d3.scaleBand()
.range([0, 2 * Math.PI]);

var y2011 = d3.scaleRadial()
.range([innerRadius2011, outerRadius2011]);

var z2011 = d3.scaleOrdinal()
...
x2011.domain(parsedData.map(function(d) { return d.pays; }));
y2011.domain([0, d3.max(parsedData, function(d) { return d.total; })]);
z2011.domain(parsedData.columns.slice(1));

var tooltip2011 = d3.select("#grapheTotal2011")
        .append("div")
        .attr("class", "tooltip")
...
.enter().append("g")
.attr("fill", function(d) { return z2011(d.key); })
.selectAll("path")
.data(function(d) { return d; })
.enter().append("path")
.attr("d", d3.arc()
    .innerRadius(function(d) { return y2011(d[0]); })
    .outerRadius(function(d) { return y2011(d[1]); })
    .startAngle(function(d) { return x2011(d.data.pays); })
    .endAngle(function(d) { return x2011(d.data.pays) + x2011.bandwidth(); })
    .padAngle(0.01)
    .padRadius(innerRadius2011))
    .on('mouseover', function(event,d) {
      //console.log(event.selection)
       tooltip2011.style("visibility", "visible")
     })
     ...
     .on('mouseout', function() {
       tooltip2011.style("visibility", "hidden")
     });

/*var label2011 = g2011.append("g")
.selectAll("g")
.data(parsedData)
.enter().append("g")
.attr("text-anchor", "middle")
.attr("transform", function(d) { return "rotate(" + ((x2011(d.pays) + x2011.bandwidth() / 2) * 180 / Math.PI - 90) + ")translate(" + innerRadius2011 + ",0)"; });

label2011.append("line")
.attr("x2", -5)
.attr("stroke", "#000");

label2011.append("text")
.attr("transform", function(d) { return (x2011(d.pays) + x2011.bandwidth() / 2 + Math.PI / 2) % (2 * Math.PI) < Math.PI ? "rotate(90)translate(0,16)" : "rotate(-90)translate(0,-9)"; })
.text(function(d) { return d.pays; });*/

var yAxis2011 = g2011.append("g")
.attr("text-anchor", "end");

var yTick2011 = yAxis2011
.selectAll("g")
.data(y2011.ticks(7).slice(1))
.enter().append("g");

yTick2011.append("circle")
.attr("fill", "none")
.attr("stroke", "#000")
.attr("stroke-opacity", 0.5)
.attr("r", y2011);

yTick2011.append("text")
.attr("x", -6)
.attr("y", function(d) { return -y2011(d); })
.attr("dy", "0.35em")
.attr("fill", "none")
.attr("stroke", "#fff")
.attr("stroke-linejoin", "round")
.attr("stroke-width2011", 3)
.text(y2011.tickFormat(10, "s"));
...
yAxis2011.append("text")
.attr("x", -6)
.attr("y", function(d) { return -y2011(y2011.ticks(10).pop()); })
.attr("dy", "-1em")
