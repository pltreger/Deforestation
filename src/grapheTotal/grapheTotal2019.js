...
var width2019 = 400, height2019 = 350;
d3.select("#grapheTotal2019")
  .append("svg")
  .attr("id","svgComparatifPaysCause2019")
  .attr("width", "100%")
  .attr("height", height2019)
  //.attr("viewBox", [0,0,800,700])
  .attr("viewBox", [300,-20,200,500])
  .attr("font-size","0.9rem")

var svg2019 = d3.select("#svgComparatifPaysCause2019"),
// width2019 = +svg2019.attr("width2019"),
// height2019 = +svg2019.attr("height2019"),
innerRadius2019 = 100,
outerRadius2019 = Math.min(width2019, height2019),
g2019 = svg2019.append("g").attr("transform", "translate(" + width2019 + "," + height2019 + ")");

var x2019 = d3.scaleBand()
.range([0, 2 * Math.PI]);

var y2019 = d3.scaleRadial()
.range([innerRadius2019, outerRadius2019]);

var z2019 = d3.scaleOrdinal()
...
x2019.domain(parsedData.map(function(d) { return d.pays; }));
y2019.domain([0, d3.max(parsedData, function(d) { return d.total; })]);
z2019.domain(parsedData.columns.slice(1));

var tooltip2019 = d3.select("#grapheTotal2019")
        .append("div")
        .attr("class", "tooltip")
...
.enter().append("g")
.attr("fill", function(d) { return z2019(d.key); })
.selectAll("path")
.data(function(d) { return d; })
.enter().append("path")
.attr("d", d3.arc()
    .innerRadius(function(d) { return y2019(d[0]); })
    .outerRadius(function(d) { return y2019(d[1]); })
    .startAngle(function(d) { return x2019(d.data.pays); })
    .endAngle(function(d) { return x2019(d.data.pays) + x2019.bandwidth(); })
    .padAngle(0.01)
    .padRadius(innerRadius2019))
    .on('mouseover', function(event,d) {
      //console.log(event.selection)
       tooltip2019.style("visibility", "visible")
     })
     ...
     .on('mouseout', function() {
       tooltip2019.style("visibility", "hidden")
     });

/*var label2019 = g2019.append("g")
.selectAll("g")
.data(parsedData)
.enter().append("g")
.attr("text-anchor", "middle")
.attr("transform", function(d) { return "rotate(" + ((x2019(d.pays) + x2019.bandwidth() / 2) * 180 / Math.PI - 90) + ")translate(" + innerRadius2019 + ",0)"; });

label2019.append("line")
.attr("x2", -5)
.attr("stroke", "#000");

label2019.append("text")
.attr("transform", function(d) { return (x2019(d.pays) + x2019.bandwidth() / 2 + Math.PI / 2) % (2 * Math.PI) < Math.PI ? "rotate(90)translate(0,16)" : "rotate(-90)translate(0,-9)"; })
.text(function(d) { return d.pays; });*/

var yAxis2019 = g2019.append("g")
.attr("text-anchor", "end");

var yTick2019 = yAxis2019
.selectAll("g")
.data(y2019.ticks(7).slice(1))
.enter().append("g");

yTick2019.append("circle")
.attr("fill", "none")
.attr("stroke", "#000")
.attr("stroke-opacity", 0.5)
.attr("r", y2019);

yTick2019.append("text")
.attr("x", -6)
.attr("y", function(d) { return -y2019(d); })
.attr("dy", "0.35em")
.attr("fill", "none")
.attr("stroke", "#fff")
.attr("stroke-linejoin", "round")
.attr("stroke-width2019", 3)
.text(y2019.tickFormat(10, "s"));
...
yAxis2019.append("text")
.attr("x", -6)
.attr("y", function(d) { return -y2019(y2019.ticks(10).pop()); })
.attr("dy", "-1em")
